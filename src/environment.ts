...
async function loadDashboardSecrets(
  ...
  const normalizedEnv = runtime.env.trim().toUpperCase()
  if (!SUPPORTED_ENVS.includes(normalizedEnv as typeof SUPPORTED_ENVS[number])) {
    throw new Error(`Invalid ENV value: ${runtime.env}. Expected one of ${SUPPORTED_ENVS.join(', ')}`)
  }
...
async function loadGlobalVariables(
  ...
  const res = await dashboardSheet.googleAccount.SHEETS.spreadsheets.values.get({
    spreadsheetId: runtime.dashboardSheetId,
...
export async function getEnvironment(): Promise<EnvironmentConfig> {
  ...
  const runtime: RuntimeEnv = {
    env: requireRuntime('ENV'),
    dashboardSheetId: requireRuntime('DASHBOARD_SHEET_ID'),
    serviceAccountEmail: requireRuntime('SHEET_SERVICE_ACCOUNT'),
    privateKey: requireRuntime('GOOGLE_PRIVATE_KEY'),
  }

  const googleUser = new GoogleUser(runtime.serviceAccountEmail, runtime.privateKey)
  const googleAccount = new GoogleAccount(googleUser)
  const dashboardSheet = new DashboardSheet(googleAccount)

  const secrets = await loadDashboardSecrets(dashboardSheet, runtime)
  const globalVariables = await loadGlobalVariables(dashboardSheet, runtime)
