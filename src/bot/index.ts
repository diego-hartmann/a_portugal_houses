...
import { normalizeInternationalPhone, splitFirstLast, titleCase } from '../lead_creation/src/flows/utils.js'
import { ConsultantControlPanel, ConsultantInDashboard, getEmptyConsultantControlPanel, getEmptyLead, Lead, Status } from '../lead_creation/src/flows/models.js'
...
function multiSelectKeyboard(options: string[], selected: string[] = [], prefix: string, doneAction: string, cancelAction: string) {
...
export function servicesKeyboard(selectedServices: string[] = []) {
  return multiSelectKeyboard(environment.globalVariablesMap.providedServices, selectedServices, 'service_toggle', 'service_done', 'service_cancel')
}

export function regionsKeyboard(selectedRegions: string[] = []) {
  return multiSelectKeyboard(environment.globalVariablesMap.regionsOfService, selectedRegions, 'region_toggle', 'region_done', 'region_cancel')
}
...
export class Bot {
  ...
  async initChatFlow() {
    ...
    this.telegramBot.on('message', async msg => {
      ...
      if (/^cancelar$/i.test(msg.text)) {
        ...
        return this.telegramBot.sendMessage(chatId, 'Entendido! Quando quiser, basta escrever "Começar" ou /start')
      }
      ...
      if (/^continuar$/i.test(msg.text)) {
        ...
        if (!s?.draft?.name) {
          ...
          return this.telegramBot.sendMessage(chatId, 'Para começarmos, como se chama? Escreva nome e sobrenome')
        }

        if (!s?.draft?.interest_services?.length) {
          this.setCurrentStep(STEP.ASK_INTEREST_SERVICES, chatId)
          return this.telegramBot.sendMessage(chatId, 'Selecione os serviços de interesse (pode escolher vários):', servicesKeyboard([]))
        }

        if (!s?.draft?.interest_regions?.length) {
          this.setCurrentStep(STEP.SELECT_REGIONS, chatId)
          return this.telegramBot.sendMessage(chatId, 'Selecione as regiões de interesse (pode escolher várias):', regionsKeyboard([]))
        }
        if (!s?.draft?.email) {
        ...
        if (!s?.draft?.phone) {
          ...
          return this.telegramBot.sendMessage(chatId, 'Qual é o indicativo do seu país?', PHONE_CC_QUICK_KEYBOARD)
        }
    ...
    this.telegramBot.on('callback_query', async query => {
      ...
      try {
        ...
        if (action === 'service_cancel') {
          ...
          return this.telegramBot.sendMessage(chatId, 'Ok. Pode selecionar novamente:', servicesKeyboard(current))
        }
        ...
        if (action === 'service_done') {
          ...
          if (!selected.length) {
            return this.telegramBot.sendMessage(chatId, 'Selecione pelo menos um serviço', servicesKeyboard(selected))
          }
          this.setCurrentStep(STEP.SELECT_REGIONS, chatId)
          return this.telegramBot.sendMessage(chatId, 'Agora selecione as regiões de interesse:', regionsKeyboard([]))
        }
        ...
        if (action === 'region_cancel') {
          ...
          return this.telegramBot.sendMessage(chatId, 'Ok. Pode selecionar novamente:', regionsKeyboard(current))
        }
        ...
        if (action === 'region_done') {
          ...
          if (!selected.length) {
            return this.telegramBot.sendMessage(chatId, 'Selecione pelo menos uma região', regionsKeyboard(selected))
          }
          this.setCurrentStep(STEP.ASK_EMAIL, chatId)
          return this.telegramBot.sendMessage(chatId, 'Para qual email enviaremos o aviso?')
        }
  ...
  private async initInitialMessage(chatId: TelegramBot.ChatId) {
    ...
    await this.telegramBot.sendMessage(chatId, 'Bem-vindo(a) à Portugal Houses Arrendamentos!\n\nPara avisarmos sobre casas disponíveis, precisaremos do seu contacto')
    this.setCurrentStep(STEP.ASK_NAME_FULL, chatId)
  ...
  private async executeActionBasedOnCurrentStep(step: STEP, msg: TelegramBot.Message) {
    const mapStepToAction: Record<STEP, () => void> = {
      ...
      [STEP.ASK_NAME_FULL]: (): void => {
        ...
        this.telegramBot.sendMessage(msg.chat.id, `É um prazer falar com você, ${titleCase(first)} ${titleCase(last)}.\nSelecione os serviços de interesse:`, servicesKeyboard([]))
      },
      [STEP.ASK_INTEREST_SERVICES]: (): void => {
        this.telegramBot.sendMessage(msg.chat.id, 'Selecione os serviços de interesse (pode escolher vários):', servicesKeyboard(this.getDraft(msg.chat.id).draft.interest_services || []))
      },
      [STEP.SELECT_REGIONS]: (): void => {
        this.telegramBot.sendMessage(msg.chat.id, 'Selecione as regiões de interesse (pode escolher várias):', regionsKeyboard(this.getDraft(msg.chat.id).draft.interest_regions || []))
      },
      [STEP.ASK_EMAIL]: (): void => {
        ...
        this.telegramBot.sendMessage(msg.chat.id, 'Qual é o código telefônico do seu país?', PHONE_CC_QUICK_KEYBOARD)
      },
      [STEP.ASK_PHONE]: (): void => {
        ...
        if (/^\+?\d{6,}$/.test(raw.replace(/\s+/g, ''))) {
          ...
          if (!normalized) {
            this.telegramBot.sendMessage(msg.chat.id, 'Hum... Esse não me parece um número válido, poderia tentar novamente?')
            return
          }
        ...
        if (/outro país|outros|outro/i.test(raw)) {
          this.telegramBot.sendMessage(msg.chat.id, 'Indique apenas os dígitos do indicativo (ex.: 34, 49, 1)')
          return
        }
        const cc = raw.replace(/\D+/g, '')
        if (!cc) {
          this.telegramBot.sendMessage(msg.chat.id, 'Envie o indicativo apenas com dígitos (ex.: 351, 55)')
          return
        }
  ...
  private async finalizeLead(chatId: TelegramBot.ChatId) {
    ...
    try {
      ...
      const getFilteredConsultantControlPanelsByMatchingServiceAndRegionWithLead = (lead: Lead, allConsultantControlPanelsInDashboard: ConsultantControlPanel[]): ConsultantControlPanel[] => {
      ...
      const getSortedConsultantControlPanelsByCommissionValue = (consultantControlPanels: ConsultantControlPanel[], desc: boolean): ConsultantControlPanel[] => {
        // TODO sort the matched consultants by their commission_value desc
        return [] // TODO return sorted list
      }

      const getRealConsultantSheetsByConsultantsInDashboard = (consultantInDashboards: ConsultantInDashboard[]) => {
      ...
      const mapRealConsultantSheetsToConsultantControlPanels = (realConsultantSheets: ConsultantInDashboard[]): ConsultantControlPanel[] => {
        return [] // TODO
      }

      const updateConsultantsInDashboardByConsultantControlPanels = (consultantControlPanels: ConsultantControlPanel[]): void => {
      ...
      const allRealConsultantSheetsFoundInGoogleSheets = getRealConsultantSheetsByConsultantsInDashboard(allConsultantsFoundInDashboard)
      const allConsultantControlPanelsFoundInGoogleSheets: ConsultantControlPanel[] = mapRealConsultantSheetsToConsultantControlPanels(allRealConsultantSheetsFoundInGoogleSheets)
      const matchedConsultantControlPanels: ConsultantControlPanel[] = getFilteredConsultantControlPanelsByMatchingServiceAndRegionWithLead(
        getEmptyLead(),
        allConsultantControlPanelsFoundInGoogleSheets,
      )
      const sortedConsultantsByCommission: ConsultantControlPanel[] = getSortedConsultantControlPanelsByCommissionValue(matchedConsultantControlPanels, true)
      const matchedConsultantControlPanel: ConsultantControlPanel = sortedConsultantsByCommission[0]!
      updateConsultantsInDashboardByConsultantControlPanels(allConsultantControlPanelsFoundInGoogleSheets)

      if (matchedConsultantControlPanel) {
        const leadRowToBeAddedToLeads: string[] = await this._leadCreator.createRowForGivenLead(lead)

        await this._leadCreator.appendRow('captured_leads!A1:K1', leadRowToBeAddedToLeads, environment.dashboardSheet.id) // TODO add 'source, matching_sheet_ids, next_sheet_index, saved_in_current_sheet_id' columns

        await this._leadCreator.appendRow('Leads!A1:K1', leadRowToBeAddedToLeads, matchedConsultantControlPanel.id)

        await this._leadCreator.appendRow('Leads History!A1:K1', leadRowToBeAddedToLeads, matchedConsultantControlPanel.id) // TODO add 'processed' column
        ...
        if (matchedConsultantControlPanel.receive_notification_on_telegram_when_new_lead) {
          matchedConsultantControlPanel.telegram_chat_ids_for_notifications.forEach(telegram_chat_id => {
            notificator.notify(
              telegram_chat_id,
              `🔥 Novo Lead!!! 🔥\n` +
                `Id: ${lead.id}\n` +
        ...
        await this._leadCreator.appendRow('orphan_leads!A1:K1', leadRowToBeAddedToOrphans, environment.dashboardSheet.id) // TODO add 'source'
        ...
        await this.telegramBot.sendMessage(chatId, 'Obrigado! Avisaremos quando houver casas disponíveis')
      }
    } catch (e) {
      console.error('finalizeLead error:', e)
      this.setCurrentStep(STEP.IDLE, chatId) // não fica preso em finalizing
      await this.telegramBot.sendMessage(chatId, 'Ocorreu um erro ao guardar. Tente novamente com /start')
    } finally {
